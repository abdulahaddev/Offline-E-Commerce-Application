...
$(document).ready(function () {
	...
	$('#search-icon').on('click', function(){
		...
		req.onsuccess = function (e) {
			...
			if (!cursor) return;
			if (cursor.value.ProductCategory == 'Wireless' && cursor.value.ProductTitle.toLowerCase().includes(searchText.toLowerCase())){
				$('#product-grid').append(
	...
	function loadProducts() {
		...
		req.onsuccess = function (e) {
			var cursor = e.target.result;
			if (!cursor) return;
			if(cursor.value.ProductCategory == 'Wireless')
			{
				$('#product-grid').append(
					`<div class= "product-container" >
				<div class="product">
